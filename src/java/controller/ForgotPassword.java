...
public class ForgotPassword extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (dao.isEmailExists(email)) {
                ...
                boolean validEmail = aDao.updateOTP(email, otp);
                if (validEmail) {
                    sendResetEmail(email, otp);
                    request.setAttribute("mess", "Password reset instruction has been sent to your email!");
                    request.getRequestDispatcher("forgetPassword.jsp").forward(request, response);
                }

            } else {
                request.setAttribute("mess", "Email doesn't exist!");
                request.getRequestDispatcher("forgetPassword.jsp").forward(request, response);
            }
    ...
    private void sendResetEmail(String email, String otp) {
        try {
            ...
            String verificationLink = "http://localhost:9999/SWP/verifyPass?email=" + email;
            String body = "Thank you for registering! Please click the following link to reset your password:\n"
